...
void clip_all_maps() {
	...
	while (tmp) {
		...
		else printf("No se incluye este mapa\n");
		tmp = tmp->next;
...
PixelPoint2D Universal2Pixel(UniversalPoint2D p) {
	PixelPoint2D np;
	np.x = (int) (H_SIZE * (p.x - win.pmin.x)) / (win.pmax.x -win.pmin.x);
	np.y = (int) (V_SIZE * (p.y - win.pmin.y)) / (win.pmax.y -win.pmin.y);
	return np;
}
