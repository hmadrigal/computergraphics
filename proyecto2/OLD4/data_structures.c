...
IntersectionList getIntersections(Point3D origin, Point3D direction) {
	double t_temp, a, b, c, disc, k=/*0.0000015*/0.000002;
	Object *tmp;
	Sphere *sph;
	Cilinder *cil;
	Polygon *pol;
		...
		switch(tmp->tipo) {
			...
			case CONE:	// Codigo del calculo de interseccion para el Cono
				...
				b = 2 * ((direction.x
						...
						* (origin.z - con->center.z - origin.z * con->direction.direction.z * con->direction.direction.z
						...
						+ -(double)( 2.0 *
							...
							*
							-pow ((double) (
								(k * con->direction.direction.x) * (origin.x - con->center.x) +
								(k * con->direction.direction.y) * (origin.y - con->center.y) +
								(k * con->direction.direction.z) * (origin.z - con->center.z)), 2.0)
				...
				c = pow((double) (origin.x - con->center.x - origin.x * con->direction.direction.x * con->direction.direction.x
						...
						(k * con->direction.direction.x) * (origin.x - con->center.x) +
						(k * con->direction.direction.y) * (origin.y - con->center.y) +
						(k * con->direction.direction.z) * (origin.z - con->center.z)
						), 2.0);
