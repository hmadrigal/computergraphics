...
Object * rotate_x_object(Object *obj, double rx) {
	...
	if(obj) {
		switch(obj->tipo) {
			...
			{
				double L;
				Point3D pu = {1.0 , 0.0 , 0.0}, pv = {0.0 , 1.0 , 0.0}, pn = {0.0 , 0.0 , 1.0}, ancla={0.0 , 0.0 , 0.0};
				spf = (Superficie2 *) obj->object;
				pu = rotate_x(pu, ancla, rx); pv = rotate_x(pv, ancla, rx); pn = rotate_x(pn, ancla, rx);
				L = sqrt ( pu.x * pu.x +  pu.y * pu.y + pu.z * pu.z); pu.x *= (1/L); pu.y *= (1/L); pu.z *= (1/L);
				L = sqrt ( pn.x * pv.x +  pv.y * pv.y + pv.z * pv.z); pv.x *= (1/L); pv.y *= (1/L); pv.z *= (1/L);
				L = sqrt ( pn.x * pn.x +  pn.y * pn.y + pn.z * pn.z); pn.x *= (1/L); pn.y *= (1/L); pn.z *= (1/L);
				spf = rotar_spf (spf, pu.x,pu.y,pu.z, pv.x,pv.y,pv.z,pn.x,pn.y,pn.z);
			}
				break;
			default:
	...
	if(obj) {
		switch(obj->tipo) {
			...
			case SUPERFICIE2:
			{
				...
				spf = (Superficie2 *) obj->object;
				pv = rotate_y(pv, ancla, ry); pv = rotate_y(pv, ancla, ry); pn = rotate_y(pn, ancla, ry);
				L = sqrt ( pu.x * pu.x +  pu.y * pu.y + pu.z * pu.z); pu.x *= (1/L); pu.y *= (1/L); pu.z *= (1/L);
				L = sqrt ( pn.x * pv.x +  pv.y * pv.y + pv.z * pv.z); pv.x *= (1/L); pv.y *= (1/L); pv.z *= (1/L);
				L = sqrt ( pn.x * pn.x +  pn.y * pn.y + pn.z * pn.z); pn.x *= (1/L); pn.y *= (1/L); pn.z *= (1/L);
				spf = rotar_spf (spf, pu.x,pu.y,pu.z, pv.x,pv.y,pv.z,pn.x,pn.y,pn.z);
	...
	if(obj) {
		switch(obj->tipo) {
			...
			case SUPERFICIE2:
			{
				...
				double L;
				spf = (Superficie2 *) obj->object;
				pn = rotate_z(pn, ancla, rz); pv = rotate_z(pv, ancla, rz); pn = rotate_z(pn, ancla, rz);
				L = sqrt ( pu.x * pu.x +  pu.y * pu.y + pu.z * pu.z); pu.x *= (1/L); pu.y *= (1/L); pu.z *= (1/L);
				L = sqrt ( pn.x * pv.x +  pv.y * pv.y + pv.z * pv.z); pv.x *= (1/L); pv.y *= (1/L); pv.z *= (1/L);
				L = sqrt ( pn.x * pn.x +  pn.y * pn.y + pn.z * pn.z); pn.x *= (1/L); pn.y *= (1/L); pn.z *= (1/L);
				spf = rotar_spf (spf, pu.x,pu.y,pu.z, pv.x,pv.y,pv.z,pn.x,pn.y,pn.z);
