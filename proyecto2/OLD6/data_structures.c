	...
	while(tmp) {
		switch(tmp->tipo) {
			...
			case CONE:	// Codigo del calculo de interseccion para el Cono
				...
				/*Yogurt*/
				+ -pow((double) (con->fk * con->fk *
								(con->direction.direction.x * direction.x +
								 con->direction.direction.y * direction.y +
								 con->direction.direction.z * direction.z) ), 2.0)	;
				b = 2 * ((direction.x
								- direction.x * con->direction.direction.x * con->direction.direction.x
								- direction.y * con->direction.direction.y * con->direction.direction.x
						...
						* (origin.z - con->center.z - origin.z * con->direction.direction.z * con->direction.direction.z
						...
						+ -(double)( 2.0 *
							...
							*
							-pow ((double) (
								(con->fk * con->fk * con->direction.direction.x) * (origin.x - con->center.x) +
								(con->fk * con->fk * con->direction.direction.y) * (origin.y - con->center.y) +
								(con->fk * con->fk * con->direction.direction.z) * (origin.z - con->center.z)), 2.0)
				...
				c = pow((double) (origin.x - con->center.x - origin.x * con->direction.direction.x * con->direction.direction.x
						...
						/*-  con->radius * con->radius*/
						/*Yogurt*/
						+ -pow ((double) (
						(con->fk * con->fk * con->direction.direction.x) * (origin.x - con->center.x) +
						(con->fk * con->fk * con->direction.direction.y) * (origin.y - con->center.y) +
						(con->fk * con->fk * con->direction.direction.z) * (origin.z - con->center.z)
						), 2.0);
			...
			case POLYGON:	// Codigo del calculo de interseccion para el poligono
				...
				if(disc > EPSILON) {
					...
					if(t_temp > EPSILON && intersectPolygon(tmp_point, pol))
						inter_list = addIntersection(inter_list, t_temp, tmp);
				}
				break;
			default:
...
Point3D getNormal(Point3D p, Object *obj) {
	...
	double L, tmp,alfa,beta,gama,cita;
	Sphere *sph;
	Cilinder *cil;
	Polygon *pol;
	Superficie2 *spf;
		...
		switch(obj->tipo) {
			...
			case CONE:	// Donde se agrega el calculo de la normal para el cono.
				...
				beta = p.y
						...
						+ con->center.z * con->direction.direction.z * con->direction.direction.y;
				/*
				gamma =  p.z + con->center.z - p.x * con->direction.direction.x * con->direction.direction.z
					+ con->center.x * con->direction.direction.x * con->direction.direction.z
...
Polygon * makePolygon(PointList pl) {
	Polygon * poly = NULL;
	Plane plane;
...
int puntosEnPlano(Plane plane, PointList pl) {
	...
	tmp = pl.first;
	while(tmp) {
		if(abs(plane.A * tmp->point.x + plane.B * tmp->point.y + plane.C * tmp->point.z  + plane.D) > EPSILON) {
...
int intersectPolygon(Point3D point, Polygon *pol) {
	...
	if ((double) abs(pol->plane.A) > (double) abs(pol->plane.B) && (double) abs(pol->plane.A) > (double) abs(pol->plane.C))
		resultado = MAX_X;
	else if((double) abs(pol->plane.B) > (double) abs(pol->plane.A) && (double) abs(pol->plane.B) > (double) abs(pol->plane.C))
		resultado = MAX_Y;
	else
		resultado = MAX_Z;
	tmp = pol->pl.first;
