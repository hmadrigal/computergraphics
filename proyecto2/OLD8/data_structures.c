		...
		switch(obj->tipo) {
			...
			case CONE:	// Donde se agrega el calculo de la normal para el cono.
				...
				tmp = 	(L = sqrt((p.x - con->center.x) * (p.x - con->center.x)
						...
						+ (p.z - con->center.z) * (p.z - con->center.z))) * cos(con->angle); // Obtengo el d
				...
				normal.z /= L;
				/*con = (Cone *) obj->object;
				tmp = 	(p.x - con->center.x) * con->direction.direction.x
						+ (p.y - con->center.y) * con->direction.direction.y
						+ (p.z - con->center.z) * con->direction.direction.z;
				Pm.x = con->center.x + tmp * con->direction.direction.x;
				Pm.y = con->center.y + tmp * con->direction.direction.y;
				Pm.z = con->center.z + tmp * con->direction.direction.z;
				R.x = p.x - Pm.x;
				...
				tmp += tan(con->angle) * L;
				normal.x = p.x - (con->center.x + tmp * con->direction.direction.x);
				normal.y = p.y - (con->center.y + tmp * con->direction.direction.y);
				normal.z = p.z - (con->center.z + tmp * con->direction.direction.z);
