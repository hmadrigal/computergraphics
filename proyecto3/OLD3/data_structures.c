	...
	while(tmp) {
		switch(tmp->tipo) {
			...
			case POLYGON:
				...
				break;
			case TORUS:
				tor = (Torus *) tmp->object;
				printf ("Torus\n");
				printf("Kd = %.4f Ka = %.4f Ks = %.4f Kn = %i\n", tmp->Kd, tmp->Ka, tmp->Ks, tmp->Kn);
				printf("R = %.4f G = %.4f B = %.4f\n", tmp->color.r, tmp->color.g, tmp->color.b);
				printf("center [%f,%f,%f]\n direction [%f,%f,%f]\nSmallRadius:%f\nBigRaius:%f\n",
				tor->center.x,tor->center.y,tor->center.z,tor->direction.x,tor->direction.y,tor->direction.z, tor->small_radius,tor->big_radius);
...
// ############################################## FIN DEL CONO
			...
			case TORUS:{
				...
				M = direction.x*direction.x + direction.y*direction.y +direction.z*direction.z;


				a = M * M;
				b = 4.0 * M * N;
				c = 4.0 * (M * M) + 2.0 * M * -2.0 * ((tor->big_radius * tor->big_radius) + (tor->small_radius*tor->small_radius))*M
				  + 4.0 * (tor->big_radius * tor->big_radius)*();
...
Color getTransparencyColor(Intersection *tmp,double kt, int level) {
	Color RGB= background, sum = {0.0,0.0,0.0};
	double T;
	...
	// Calculo de Transparencia
	T = kt * tmp->obj->Kt;
	RGB.r *= kt; RGB.g *= kt; RGB.b *= kt;
	RGB.g = T * tmp->obj->color.g;
	RGB.b = T * tmp->obj->color.b;

	sum = getTransparencyColor(tmp->next,T, level-1) ;
	sum.r+=RGB.r;
	sum.g+=RGB.g;
	sum.b+=RGB.b;
	printf ("Kt%f->[%i]\t",kt,level);
	return sum;
}
...
Color getColor(Point3D origin, Point3D direction) {
	...
	Color color = background , transparency={0.0,0.0,0.0};
	Intersection * tmp;
	Point3D pi, normal;
	...
	if(tmp = inter_list.first) {
		...
		//Obtiene el Color de Transparencia
		if (! phong)
			P = 0.0;
		if (! lights)
			I = 1.0;
