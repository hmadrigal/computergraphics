...
Color getColor_AntiAliasign(Color a, Color b , Color c, Color d, double i, double j , int level, double factor) {
	...
	//Calcula la diferencia entre los colores
 	L = 	(1.0 / sqrt(  pow(a.r-b.r,2.0)+ pow(a.g-b.g,2.0)+pow(a.b-b.b,2.0))) +
		(1.0 / sqrt(  pow(a.r-c.r,2.0)+ pow(a.g-c.g,2.0)+pow(a.b-c.b,2.0))) +
		(1.0 / sqrt(  pow(a.r-d.r,2.0)+ pow(a.g-d.g,2.0)+pow(a.b-d.b,2.0))) +
		(1.0 / sqrt(  pow(b.r-c.r,2.0)+ pow(b.g-c.g,2.0)+pow(b.b-c.b,2.0))) +
		(1.0 / sqrt(  pow(b.r-d.r,2.0)+ pow(b.g-d.g,2.0)+pow(b.b-d.b,2.0))) ;
	L*=2;
			...
			for (i=0,k=0;i<H_SIZE;i++,k++){
				...
				*/
				...
				Color rate = {	(actfila[k].r+actfila[k+1].r+c.r+d.r) /4.0,
					...
					buffer[i][j] = getColor_AntiAliasign(actfila[k], actfila[k+1] , c, d, i, j , nivel_anti_aliasign, 0.5);
				}
				//Pinta el pixel
				glBegin(GL_POINTS);
				glColor3f(buffer[i][j].r,buffer[i][j].g,buffer[i][j].b);
