...
%token ALTURA, BRILLO, TRANSPARENCIA, PHONG, RANGO
...
dato_escena:
	...
	| COLOR_FONDO NUMERO NUMERO NUMERO {setBackground($<ldval>2, $<ldval>3, $<ldval>4);}
...
luz:
	...
	INTENSIDAD NUMERO {
		luces = addLight(luces, $<ldval>13,
							$<ldval>3 / 255.0, $<ldval>4 / 255.0, $<ldval>5 / 255, 
							$<ldval>8, $<ldval>9, $<ldval>10);
	};
...
objeto:
	...
	O2 NUMERO
	O3 NUMERO {
		tmp_obj->color.r = $<ldval>3 / 255;
		tmp_obj->color.g = $<ldval>4 / 255;
		tmp_obj->color.b = $<ldval>5 / 255;
		tmp_obj->Kd = $<ldval>7;
		tmp_obj->Ka = $<ldval>9;
		tmp_obj->Ks = $<ldval>11;
		tmp_obj->Kn = (int) $<ldval>13;
		tmp_obj->Kt = $<ldval>15;
		tmp_obj->fO1 = $<ldval>17;
		tmp_obj->fO2 = $<ldval>19;
		tmp_obj->fO3 = $<ldval>21;
	}
	objeto_real
	planos
	transformaciones
...
poligono:
	...
	ANCLA '('NUMERO NUMERO NUMERO')' {
		ultimo_centro.x = (double) $<ldval>3;
		ultimo_centro.y = (double) $<ldval>4;
		ultimo_centro.z = (double) $<ldval>5;
...
transformaciones:
	TRANSFORMACIONES {printf("Cargaremos Transformaciones\n");}lista_transformaciones{printf("Cargamos Trans\n");}
	| ;
...
transformacion:
	...
	| TRASLADAR '[' NUMERO NUMERO NUMERO ']' {
	  	if(tmp_obj->tipo == DISK)
			trasladar_disco($<ldval>3, $<ldval>4, $<ldval>5);
...
planos:
	PLANOS_DE_CORTE {printf("Cargaremos Planos de Corte\n");}lista_planos{printf("Cargamos los planos\n");}
	| ;
...
plano_de_corte:
	PLANO_TRANSFORMADO_X '+' {ubica_plano_x();} '{' trans_plano '}' {
	...
	| PLANO_TRANSFORMADO_Y '+' {ubica_plano_y();} '{' trans_plano '}' {
		puntos_planos = agregaPlano(puntos_planos, ptmp1, ptmp2, ptmp3, TRUE);
	}
	| PLANO_TRANSFORMADO_Y '-' {ubica_plano_y();} '{' trans_plano '}' {
		puntos_planos = agregaPlano(puntos_planos, ptmp1, ptmp2, ptmp3, FALSE);
	}
	| PLANO_TRANSFORMADO_Z '+' {ubica_plano_z();} '{' trans_plano '}' {
		puntos_planos = agregaPlano(puntos_planos, ptmp1, ptmp2, ptmp3, TRUE);
	}
	| PLANO_TRANSFORMADO_Z '-' {ubica_plano_z();} '{' trans_plano '}' {
		puntos_planos = agregaPlano(puntos_planos, ptmp1, ptmp2, ptmp3, FALSE);
...
transforma_plano :
	ROTAR_X NUMERO {
						ptmp1 = rotate_x(ptmp1, ultimo_centro, $<ldval>2 / RADIAN);
						ptmp2 = rotate_x(ptmp2, ultimo_centro, $<ldval>2 / RADIAN);
						ptmp3 = rotate_x(ptmp3, ultimo_centro, $<ldval>2 / RADIAN);}
	| ROTAR_Y NUMERO {
						ptmp1 = rotate_y(ptmp1, ultimo_centro, $<ldval>2 / RADIAN);
						ptmp2 = rotate_y(ptmp2, ultimo_centro, $<ldval>2 / RADIAN);
						ptmp3 = rotate_y(ptmp3, ultimo_centro, $<ldval>2 / RADIAN);}
	| ROTAR_Z NUMERO {
						ptmp1 = rotate_z(ptmp1, ultimo_centro, $<ldval>2 / RADIAN);
						ptmp2 = rotate_z(ptmp2, ultimo_centro, $<ldval>2 / RADIAN);
						ptmp3 = rotate_z(ptmp3, ultimo_centro, $<ldval>2 / RADIAN);}
	| TRASLADAR '[' NUMERO NUMERO NUMERO ']' {
						ptmp1 = translate(ptmp1, $<ldval>3, $<ldval>4, $<ldval>5);
						ptmp2 = translate(ptmp2, $<ldval>3, $<ldval>4, $<ldval>5);
						ptmp3 = translate(ptmp3, $<ldval>3, $<ldval>4, $<ldval>5);
		}
	| ;
