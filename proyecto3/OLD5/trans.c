	...
	if(obj) {
		switch(obj->tipo) {
			...
			case CONE:
				...
				con->north = rotate_x(con->north, con->center, rx);
			...
			case CILINDER:
				...
				cil->north = rotate_x(cil->north, cil->center, rx);
				cil->greenwich = rotate_y(cil->greenwich, cil->center, rx);
				if(puntos_planos)
	...
	if(obj) {
		switch(obj->tipo) {
			...
			case CONE:
				...
				con->north = rotate_y(con->north, con->center, ry);
			...
			case CILINDER:
				...
				cil->north = rotate_y(cil->north, cil->center, ry);
	...
	if(obj) {
		switch(obj->tipo) {
			...
			case CONE:
				...
				con->north = rotate_z(con->north, con->center, rz);
...
void rotar_disco_x(double angle) {
	...
	Udisk.y /= L;
	Udisk.z /= L;
	L = sqrt(Vdisk.x * Vdisk.x + Vdisk.y * Vdisk.y + Vdisk.z * Vdisk.z);
	Vdisk.x /= L;
	Vdisk.y /= L;
	Vdisk.z /= L;*/
