...
Color getColor_AntiAliasign(Color a, Color b , Color c, Color d, double i, double j , int level, double factor) {
	double L;
	Color rate = {(a.r+b.r+c.r+d.r) /(double) 4.0,
			(a.g+b.r+c.g+d.g) /(double) 4.0,
			(a.b+b.b+c.b+d.b) /(double) 4.0};
	...
	if (L < EPSILIN_ANTIALIASING) {
		...
		c1 = getColor_AntiAliasign(a,e,f,g, i,j,level-1, factor / (double) 2.0);
		c2 = getColor_AntiAliasign(e,b,g,h, i+factor,j,level-1, factor / (double) 2.0);
		c3 = getColor_AntiAliasign(f,g,c,k, i,j-factor,level-1, factor / (double) 2.0);
		c4 = getColor_AntiAliasign(g,h,k,d, i+factor,j-factor,level-1, factor / (double) 2.0 );

		// Promedio Metapixeles
		//printf ("[%f][%f]Cuadrante en Nivel (%i) \n",i,j,level);
		rate.r = (c1.r+c2.r+c3.r+c4.r) / (double) 4.0;
		rate.g = (c1.g+c2.g+c3.g+c4.g) / (double) 4.0;
		rate.r = (c1.b+c2.b+c3.b+c4.b) / (double) 4.0;
		...
		for (j=lastX;j<V_SIZE;j++){
			...
			for (i=0,k=0;i<H_SIZE;i++,k++){
				...
				*/
				...
				if (L < EPSILIN_ANTIALIASING) {
				Color rate = {	(actfila[k].r+actfila[k+1].r+c.r+d.r) /(double) 4.0,
						(actfila[k].g+actfila[k+1].r+c.g+d.g) /(double) 4.0,
						(actfila[k].b+actfila[k+1].b+c.b+d.b) /(double) 4.0};
					buffer[i][j].r = rate.r;
					buffer[i][j].g = rate.g;
					buffer[i][j].b = rate.b;
					...
					buffer[i][j] = getColor_AntiAliasign(actfila[k], actfila[k+1] , c, d, i, j , nivel_anti, (double) 0.5);
