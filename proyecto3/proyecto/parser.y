...
%token ACOTADO, SIN_ACOTAR, TIPO, CALADO
%token CUAD_A, CUAD_B, CUAD_C, CUAD_D, CUAD_E, CUAD_F, CUAD_G, CUAD_H, CUAD_J, CUAD_K, CUADRATICA
...
esfera:
	...
	NORTE '[' NUMERO NUMERO NUMERO ']' {
		...
		sph->greenwich.x =(double) $<ldval>12;
		sph->greenwich.y =(double) $<ldval>13;
		sph->greenwich.z =(double) $<ldval>14;
		sph->north.x =(double) $<ldval>18;
		sph->north.y =(double) $<ldval>19;
		sph->north.z =(double) $<ldval>20;
...
cuadratica:
	CUADRATICA '{'
		...
		CUAD_J '=' NUMERO ','
		CUAD_K '=' NUMERO '}' {
			cuad = (Cuadratica *) calloc(1, sizeof(Cuadratica));
			cuad->A = (double) $<ldval>5;
			cuad->B = (double) $<ldval>9;
			cuad->C = (double) $<ldval>13;
			cuad->D = (double) $<ldval>17;
			cuad->E = (double) $<ldval>21;
			cuad->F = (double) $<ldval>25;
			cuad->G = (double) $<ldval>29;
			cuad->H = (double) $<ldval>33;
			cuad->J = (double) $<ldval>37;
			cuad->K = (double) $<ldval>41;
			tmp_obj->tipo = QUADRIC;
			tmp_obj->object = cuad;
			cuad = NULL;
			ultimo_centro.x = ultimo_centro.y = ultimo_centro.z = 0.0;
			printf("Cargamos la cuadratica\n");
			//exit(0);
		};
...
transformacion:
	...
	| ESCALAR '[' NUMERO NUMERO NUMERO ']' {
		tmp_obj = scale_object(tmp_obj, (double) $<ldval>3, (double) $<ldval>4, (double) $<ldval>5);}
...
tipo_mapa:
	STRETCH	 {mapa->tipo=FILL_STRETCH;}
	| TILED NUMERO NUMERO {mapa->tipo=FILL_TILED; mapa->ancho = $<ldval>2; mapa->alto = $<ldval>3;}
	;
...
image:
	...
	| ALTURA IMAGEN {mapa = (MapaAlturaObjeto *) calloc (1,sizeof(MapaAlturaObjeto));} NUMERO RANGO NUMERO NUMERO tipo_mapa
	{
		tmp_obj->mapa_altura = (int) $<ldval>4;
		mapa->min = (double) $<ldval>6;
		mapa->max = (double) $<ldval>7;
		mapa->range_size = fabs(mapa->min - mapa->max);
		printf("MAXIMO %.4f MINIMO %.4f\n", mapa->max, mapa->min);
		tmp_obj->mapa = mapa;
		mapa = NULL;
	}
	| BRILLO IMAGEN NUMERO
	| TRANSPARENCIA IMAGEN NUMERO
	| PHONG IMAGEN NUMERO
	| CALADO IMAGEN {mapa = (MapaAlturaObjeto *) calloc (1,sizeof(MapaAlturaObjeto));} NUMERO tipo_mapa {
			tmp_obj->mapa_calado = (int) $<ldval>4;
			tmp_obj->mapa = mapa;
			mapa = NULL;
		};
