...
//Verifica si un punto esta calado
int isCalated (Object *obj , Point3D origen, Point3D dir, int t) {
	if (obj->mapa_calado>=0) {
		Point3D pi;
		UV_st index;
		double l;
		int x,y;
		pi.x = origen.x + t * dir.x ;
		pi.y = origen.y + t * dir.y ;
		pi.z = origen.z + t * dir.z ;

		//l = sqrt (origen.x*origen.x + origen.y*origen.y + origen.z*origen.z);
		//pi.x = pi.x / l; pi.y = pi.y / l; pi.z = pi.z / l;
		index = getUV(obj, pi , 2);
		x = (int)(index.u * (arreglo_grises[obj->mapa_calado]->width - 1));
		y = (int)(index.v * (arreglo_grises[obj->mapa_calado]->height -1));
		//printf("X = %i Y = %i\n", x, y);
		//printf("X = %.4f Y = %.4f\n", index.u, index.v);

		//Si la poscion es cero regresa false
		if (arreglo_grises[obj->mapa_calado]->grises[x][arreglo_grises[obj->mapa_calado]->height - y -1 ] == 0.0)
			return FALSE;
	}
	return TRUE;
}
			...
			case SPHERE:
				...
				if(fabs(disc) < EPSILON) {	// Hay una solucion
					...
					if (t_temp > EPSILON && isAccepted(t_temp, origin, direction, tmp->plane_list)
						&& isCalated (tmp , origin, direction , t_temp) )
...
// ############################################## INICIO CONO
				...
				if(fabs(disc) < EPSILON) {	// Hay una solucion
					...
					if (t_temp > EPSILON && isAccepted(t_temp, origin, direction, tmp->plane_list)
						&& ! estaFuera(t_temp, origin, direction, tmp)
						&& isCalated (tmp , origin, direction , t_temp) )
						inter_list = addIntersection(inter_list, t_temp, tmp);
				}
				else if (disc > 0) {			// Hay dos soluciones
...
// ############################################## FIN DEL CONO
			...
			case CILINDER:
				...
				if(fabs(disc) < EPSILON) {	// Hay una solucion
					...
					if (t_temp > EPSILON && ! estaFuera(t_temp, origin, direction, tmp)
						...
						&& isCalated (tmp , origin, direction , t_temp) ) {
						inter_list = addIntersection(inter_list, t_temp, tmp);
						//printf("Interseccion con el Cilindro %.4f\n", t_temp);
						}
				}
				else if (disc > 0) {			// Hay dos soluciones
					t_temp = (double) (-b + sqrt(disc)) / (2 * a);
			...
			case DISK:	// Codigo del calculo de interseccion para el poligono
				...
				break;
			case QUADRIC:	// Codigo del calculo de interseccion para el poligono
				cuad = (Cuadratica *) tmp->object;
				//printf("Soy cuadratica\n");
				a = cuad->A * direction.x * direction.x
					+ cuad->B * direction.y * direction.y
					+ cuad->C * direction.z * direction.z
					+ 2 *(cuad->D * direction.x * direction.y)
					+ 2 *(cuad->E * direction.y * direction.z)
					+ 2 *(cuad->F * direction.x * direction.z);
				b = 2 * (
					cuad->A * origin.x * direction.x
					+ cuad->B * origin.y * direction.y
					+ cuad->C * origin.z * direction.z
					+ cuad->D * origin.x * direction.y
					+ cuad->D * origin.y * direction.x
					+ cuad->E * origin.y * direction.z
					+ cuad->E * origin.z * direction.y
					+ cuad->F * origin.z * direction.x
					+ cuad->F * origin.x * direction.z
					+ cuad->G * direction.x
					+ cuad->H * direction.y
					+ cuad->J * direction.z
					);
				c = cuad->A * origin.x * origin.x
					+ cuad->B * origin.y * origin.y
					+ cuad->C * origin.z * origin.z
					+ 2 * (	cuad->D * origin.x * origin.y
						+ cuad->E * origin.y * origin.z
						+ cuad->F * origin.x * origin.z
						+ cuad->G * origin.x
						+ cuad->H * origin.y
						+ cuad->J * origin.z )
					+ cuad->K;
				...
				if(fabs(disc) < EPSILON) {	// Hay una solucion
					...
					if (t_temp > EPSILON
						...
						/*&& isCalated (tmp , origin, direction , t_temp)*/ ) {
						inter_list = addIntersection(inter_list, t_temp, tmp);
						//printf("Interseccion con cuadratica %.4f\n", t_temp);
						}
				}
				else if (disc > 0) {			// Hay dos soluciones
					t_temp = (double) (-b + sqrt(disc)) / (2 * a);
				...
				break;
			default:
				printf("Objeto sin ecuacion de intersecciones %i\n", tmp->tipo);
				break;
		}
		tmp = tmp->next;
	}
...
int estaFuera(double t, Point3D origin, Point3D direction, Object *obj) {
	...
	if(obj) {
		switch(obj->tipo) {
			...
			case CONE:
				con = (Cone *) obj->object;
				if (con->distancia > 0.0) {
					double D;
					Point3D pi = {origin.x + direction.x * t,
								origin.y + direction.y * t,
								origin.z + direction.z * t};
					D = (pi.x - con->center.x) * con->direction.direction.x
						+ (pi.y - con->center.y) * con->direction.direction.y
						+ (pi.z - con->center.z) * con->direction.direction.z;
					if(D < 0.0 || D > con->distancia)
						fuera = TRUE;
				}
				break;
		}
...
UV_st getUV(Object *obj , Point3D pi, char mapeo) {
	...
	if (obj) {
		...
		switch (obj->tipo) {
			...
			case DISK: {
				...
				//d1 = MIN(fabs(L.x*dis->U.x + L.y*dis->U.y + L.z*dis->U.z), 1.0);
				//d2 = MIN(fabs(L.x*dis->V.x + L.y*dis->V.y + L.z*dis->V.z), 1.0);
			} break;
			case CILINDER: {		// stretch_height_default, stretch_width_default;
				...
				// Ya tenemos la distancia calculada
				ancho = 2 * PI;
				//N Pm = center + t * direccion
				// N = pi - Pm	-> Se normaliza con el radio
				//d1 = acos(N (punto) greenwich)
				d1 = acos(((pi.x - (cil->center.x + cil->direction.direction.x * d2)) / cil->radius) * cil->greenwich.x
						+ ((pi.y - (cil->center.y + cil->direction.direction.y * d2)) / cil->radius) * cil->greenwich.y
						+ ((pi.z - (cil->center.z + cil->direction.direction.z * d2)) / cil->radius) * cil->greenwich.z);
				if(pi.x * cil->plane.A + pi.y * cil->plane.B + pi.z * cil->plane.C + cil->plane.D > 0.0)
					d1 = ancho - d1;

				if (obj->textura && obj->textura->tipo == FILL_TILED && mapeo == 1) {
					d1 *= cil->radius * cil->radius;
					ancho = 2 * PI * cil->radius * cil->radius;
				} else if(obj->mapa && mapeo == 2 && obj->mapa->tipo == FILL_TILED) {
					d1 *= cil->radius * cil->radius;
					ancho = 2 * PI * cil->radius * cil->radius;
				}
			} break;
		case SPHERE:{
			...
			Point3D P;
			double d;
			Point3D normal = getNormal(pi,obj);
			//printf("Norte %.4f %.4f %.4f\n", sp->north.x, sp->north.y, sp->north.z);
			//printf("Normal %.4f %.4f %.4f\n", normal.x, normal.y, normal.z);
			// d = North Dot_Product  Normal
			//if (sp->north.x != normal.x && sp->north.y != normal.y && sp->north.z != normal.z) {
				d = (sp->north.x * normal.x + sp->north.y * normal.y + sp->north.z * normal.z);
				//printf("D = %.5f\n", d);
				//Calcula V
				struc.v = fabs(1 - (acos (d)/PI)); // d = North Dot_Product  Normal
				//printf("V = %.5f\n", struc.v);
				//printf("V = %.5f\n", struc.v);
				...
				//printf("Sombra %.4f %.4f %.4f\n", P.x, P.y, P.z);
				//printf("Greenwich %.4f %.4f %.4f\n", sp->greenwich.x, sp->greenwich.y, sp->greenwich.z);
				//Calcula U
				d = P.x * sp->greenwich.x + P.y * sp->greenwich.y + P.z * sp->greenwich.z;
				struc.u = acos (d) / (2.0 *  PI);

				d = sp->plane.A*pi.x + sp->plane.B*pi.y + sp->plane.C*pi.z + sp->plane.D;
				if (d>=0.0) struc.u = 1.0 - struc.u ;
				//printf("U = %.5f -- V = %.5f\n", struc.u, struc.v);
				struc.u = MIN(1.0, struc.u);
				struc.v = MIN(1.0, struc.v);
			//} else struc.u = struc.u = 1.0;
			return struc;
		}break;
			case CONE: {		// stretch_height_default, stretch_width_default;
				Cone *con = (Cone *) obj->object;
				double lon, alfa, h;
				Point3D N;

				d2 = (pi.x - con->center.x) * con->direction.direction.x
					+ (pi.y - con->center.y) * con->direction.direction.y
					+ (pi.z - con->center.z) * con->direction.direction.z;
				if(con->distancia > 0.0)	// Esta acotado
					alto = 2 * con->distancia;
				else {
					printf("El cilindro no esta acotado\n");
					alto = 2 * stretch_height_default;
				}
				// Ya tenemos la distancia calculada
				ancho = 2 * PI;
				//N Pm = center + t * direccion
				// N = pi - Pm	-> Se normaliza con el radio
				//d1 = acos(N (punto) greenwich)
				N.x = pi.x - (con->center.x + con->direction.direction.x * d2);
				N.y = pi.y - (con->center.y + con->direction.direction.y * d2);
				N.z = pi.z - (con->center.z + con->direction.direction.z * d2);
				lon = sqrt(N.x * N.x + N.y * N.y + N.z * N.z);
				N.x /= lon; N.y /= lon; N.z /= lon;
				alfa = acos(N.x * con->greenwich.x
						+ N.y * con->greenwich.y
						+ N.z * con->greenwich.z);
				if(pi.x * con->plane.A + pi.y * con->plane.B + pi.z * con->plane.C + con->plane.D > 0.0)
					alfa = ancho - alfa;
				// Ahora hay que sacar las proporciones de d1 y d2
				// d2 funciona como la altura. Tenemos el punto [0.0, -d2] el cual hay que rotar d1 radianes.
				/*if(obj->textura->tipo == FILL_TILED) {
					d1 *= cil->radius * cil->radius;
					ancho = 2 * PI * cil->radius * cil->radius;
					//printf("Distancia %.4f Circunferencia %.4f\n", d1, ancho);
				}*/
				ancho = 2 * con->distancia;
				h = -con->distancia + d2;
				d1 = -(h * sin(alfa)) + con->distancia;
				d2 = h * cos(alfa) + con->distancia;
			} break;
		}

		//Si quieres Textura o Mapa
		switch (mapeo) {
			case 1: //Textura
			...
			break;
			case 2:
				if (obj->mapa->tipo == FILL_TILED) {
					struc.u = (double) ((int) d1 % (int) obj->mapa->ancho) / obj->mapa->ancho;
					struc.v = (double) ((int) d2 % (int) obj->mapa->alto)  / obj->mapa->alto;
				} else { //if (obj->textura->tipo == FILL_STRETCH) {
					struc.u = d1 / ancho;
					struc.v = d2 / alto;
				}
	...
	struc.u = MIN(struc.u, 1.0);
	struc.v = MIN(struc.v, 1.0);
	return struc;
}
...
Color getColor(Point3D origin, Point3D direction, int nivel) {
	...
	//udf
	contador++;
	if (contador == 2){ contador = 0; return color; };
	...
	if(tmp = inter_list.first) {
		...
		normal = bumpMapNormal(pi, getNormal(pi,  tmp->obj), tmp->obj);
...
Color getTransparenciaMejorada(int nivel, Point3D point, Point3D origin, Point3D direction, Intersection *next_int, double Fatt) {
	...
	if(next_int && next_int->obj->fO3 > EPSILON && nivel ) {
...
Point3D getNormal(Point3D p, Object *obj) {
	...
	//normal = bumpMapNormal(p, normal, obj);
	return normal;
}

// Recalcula la nueva normal con el bump mapping
Point3D bumpMapNormal(Point3D pi, Point3D normal, Object *obj) {
	Point3D new_normal = normal;
	if(obj && obj->mapa_altura >= 0) {
		UV_st uv;
		int i, j, ancho, alto;
		double tmp1, tmp2, intervalo, ij_val, k1, k2;
		Polygon *pol;
		Cilinder *cil;
		Cone *con;
		Sphere *sph;
		...
		uv = getUV(obj, pi , 2);
		//printf("U = %.6f --- V = %.6f\n", uv.u, uv.v);
		i = (int) ((ancho = (arreglo_grises[obj->mapa_altura]->width - 1)) * uv.u);
		j = (int) ((alto = (arreglo_grises[obj->mapa_altura]->height - 1)) * uv.v);

		// Calculo el tamanho de la variacion
		if(intervalo = obj->mapa->range_size) {
			// Subimos o bajamos la normal
			ij_val = arreglo_grises[obj->mapa_altura]->grises[i][alto - j];
			tmp1 = obj->mapa->min + ij_val * intervalo;
			//printf("Normal Vieja %.5f %.5f %.5f\n", normal.x, normal.y, normal.z);
			new_normal.x = normal.x + tmp1 * normal.x;
			new_normal.y = normal.y + tmp1 * normal.y;
			new_normal.z = normal.z + tmp1 * normal.z;
			//printf("Normal Nueva %.5f %.5f %.5f\n", new_normal.x, new_normal.y, new_normal.z);
			//printf("I %i -- J %i\n", i, j);
			tmp1 = arreglo_grises[obj->mapa_altura]->grises[(! i)? 0 : i - 1][alto - j];
			tmp1 = obj->mapa->min + tmp1 * intervalo;
			tmp2 = arreglo_grises[obj->mapa_altura]->grises[((i + 1) >= ancho)? i : i + 1][alto - j];
			tmp2 = obj->mapa->min + tmp2 * intervalo;
			k1 = (double) (tmp2 - tmp1) / 2;		// Este se usa con U
			tmp1 = arreglo_grises[obj->mapa_altura]->grises[i][(! j)? alto : alto - (j - 1)];
			tmp1 = obj->mapa->min + tmp1 * intervalo;
			tmp2 = arreglo_grises[obj->mapa_altura]->grises[i][((j + 1) >= alto)? alto - j : alto - (j + 1)];
			tmp2 = obj->mapa->min + tmp2 * intervalo;
			k2 = (double) (tmp2 - tmp1) / 2;

			switch(obj->tipo) {
				case POLYGON:
					pol = (Polygon *) obj->object;
					new_normal.x = new_normal.x - k1 * pol->U.x - k2 * pol->V.x;
					new_normal.y = new_normal.y - k1 * pol->U.y - k2 * pol->V.y;
					new_normal.z = new_normal.z - k1 * pol->U.z - k2 * pol->V.z;
					// Ahora la normalizamos
					tmp1 = sqrt(new_normal.x * new_normal.x + new_normal.y * new_normal.y + new_normal.z * new_normal.z);
					new_normal.x /= tmp1;
					new_normal.y /= tmp1;
					new_normal.z /= tmp1;
					break;
				case CILINDER:
					cil = (Cilinder *) obj->object;
					newU.x = cil->direction.direction.y * new_normal.z - cil->direction.direction.z * new_normal.y;
					newU.y = cil->direction.direction.z * new_normal.x - cil->direction.direction.x * new_normal.z;
					newU.z = cil->direction.direction.x * new_normal.y - cil->direction.direction.y * new_normal.x;
					new_normal.x = new_normal.x - k1 * newU.x - k2 * cil->direction.direction.x;
					new_normal.y = new_normal.y - k1 * newU.y - k2 * cil->direction.direction.y;
					new_normal.z = new_normal.z - k1 * newU.z - k2 * cil->direction.direction.z;
					// Ahora la normalizamos
					tmp1 = sqrt(new_normal.x * new_normal.x + new_normal.y * new_normal.y + new_normal.z * new_normal.z);
					new_normal.x /= tmp1;
					new_normal.y /= tmp1;
				...
				case CONE:
					con = (Cone *) obj->object;
					newU.x = con->direction.direction.y * new_normal.z - con->direction.direction.z * new_normal.y;
					newU.y = con->direction.direction.z * new_normal.x - con->direction.direction.x * new_normal.z;
					newU.z = con->direction.direction.x * new_normal.y - con->direction.direction.y * new_normal.x;
					new_normal.x = new_normal.x - k1 * newU.x - k2 * con->direction.direction.x;
					new_normal.y = new_normal.y - k1 * newU.y - k2 * con->direction.direction.y;
					new_normal.z = new_normal.z - k1 * newU.z - k2 * con->direction.direction.z;
					// Ahora la normalizamos
					tmp1 = sqrt(new_normal.x * new_normal.x + new_normal.y * new_normal.y + new_normal.z * new_normal.z);
					new_normal.x /= tmp1;
					new_normal.y /= tmp1;
				...
				case SPHERE:
					sph = (Sphere *) obj->object;
					newU.x = sph->north.y * new_normal.z - sph->north.z * new_normal.y;
					newU.y = sph->north.z * new_normal.x - sph->north.x * new_normal.z;
					newU.z = sph->north.x * new_normal.y - sph->north.y * new_normal.x;
					newV.x = newU.y * new_normal.z - newU.z * new_normal.y;
					newV.y = newU.z * new_normal.x - newU.x * new_normal.z;
					newV.z = newU.x * new_normal.y - newU.y * new_normal.x;
					new_normal.x = new_normal.x - k1 * newU.x - k2 * newV.x;
					new_normal.y = new_normal.y - k1 * newU.y - k2 * newV.y;
					new_normal.z = new_normal.z - k1 * newU.z - k2 * newV.z;
					// Ahora la normalizamos
					tmp1 = sqrt(new_normal.x * new_normal.x + new_normal.y * new_normal.y + new_normal.z * new_normal.z);
					new_normal.x /= tmp1;
					new_normal.y /= tmp1;
...
IP_st getIntensityAndPhong(Point3D point, Point3D direction, Point3D normal, Object *obj) {
	...
	if(light)
		while(light) {
			...
			cos_alfa =(double) MAX((L.x * normal.x + L.y * normal.y + L.z * normal.z), 0.0);
			// Para ver si hay sombra
			tmp_int = (int_list = getIntersections(point, L)).first;
			if(tmp_int /*&& nivel_transparencia*/) {
				//if(nivel_transparencia) {
					multi_Kt = 1.0;
					while(tmp_int){
...
int puntosEnPlano(Plane plane, PointList pl) {
	...
	pol_ancho = fabs(max.x - min.x);
	pol_alto  = fabs(max.y - min.y);

	//Guarda la posicion de la esquina
	pol_p0.x = min.x; pol_p0.y = min.y; pol_p0.z=0.0;
	//ultimo_centro = pol_p0;
	printf("------------------- Po = %.4f %.4f %.4f\n", pol_p0.x, pol_p0.y, pol_p0.z);
	printf("++++++++++++++ Ultimo Centro = %.4f %.4f %.4f\n", ultimo_centro.x, ultimo_centro.y, ultimo_centro.z);
	return TRUE;
}
...
Mapa_Grises * loadGris(char *file) {
	...
	if ((handle = fopen(file, "r+")) && fread(&header, sizeof(AVSHeader), 1, handle) && esAVS(handle, header)) {
		...
		for (j = 0; j < image->height; j++)
			for (i = 0; i < image->width; i++) {
				fread(&pixel, sizeof(AVSPixel), 1, handle);
				image->grises[i][j] = (double) (pixel.r + pixel.g + pixel.b) / 765;
			}
		fclose(handle);
	} else {
		if(handle) fclose(handle);
