...
UV_st getUV(Object *obj , Point3D pi) {
	UV_st struc = {-1.0, -1.0};
	...
	if (obj->textura) {
		...
		switch (obj->tipo) {
			case POLYGON: {
				...
				d1 = MIN(fabs(L.x*pol->U.x + L.y*pol->U.y + L.z*pol->U.z) -EPSILON, ancho +EPSILON);
				d2 = MIN(fabs(L.x*pol->V.x + L.y*pol->V.y + L.z*pol->V.z) -EPSILON, alto +EPSILON);
...
Color getObjectColor (Object *obj , Point3D pi) {
	UV_st punto;
	int x,y;
	...
	if (obj->textura) {
		punto = getUV(obj, pi);
		x = (int)((punto.u * (arreglo_texturas[obj->textura->index]->width)));
		y = (int)((punto.v * (arreglo_texturas[obj->textura->index]->height)));
		//printf("U - V ** %.3f %.3f\n", punto.u, punto.v);
		//printf("Accesa %i %i\n", x, (arreglo_texturas[obj->textura->index]->height-1) - y);
		color = arreglo_texturas[obj->textura->index]->image[x][arreglo_texturas[obj->textura->index]->height-y];
		//printf("Acceso\n", x, (arreglo_texturas[obj->textura->index]->height-1) - y);
	}
...
Color getColor(Point3D origin, Point3D direction, int nivel) {
	...
	Intersection * tmp, *tmp2;
	Point3D pi, normal, V, R, Nprima;
	...
	if(tmp = inter_list.first) {
		...
		//color = tmp->obj->color;
		color = getObjectColor(tmp->obj,pi);
		...
		if(nivel && tmp->obj->fO2 != 0.0) {
			...
			Nprima.x = 2 * prod_tmp * normal.x;
			Nprima.y = 2 * prod_tmp * normal.y;
			Nprima.z = 2 * prod_tmp * normal.z;
			R.x = Nprima.x - V.x;
			R.y = Nprima.y - V.y;
			R.z = Nprima.z - V.z;
			reflejo = getColor(pi, R, nivel - 1);
		}
		// Por aqui se calcula la tasparencia
		...
		if(tmp->obj->Kt > EPSILON && nivel_transparencia) {
			tmp2 = tmp->next;
			while(tmp2 && tmp->obj == tmp2->obj)
				tmp2=tmp2->next;
			transparencia = getTransparencia(tmp2, nivel_transparencia - 1, origin, direction);
			transparencia.r *= tmp->obj->Kt;
			transparencia.g *= tmp->obj->Kt;
			transparencia.b *= tmp->obj->Kt;
		}
...
/*
	Para la transparencia lo que se necesita es tener la lista de intersecciones.
	Si hay nivel de transparencia, se llama esta funcion.
	Se busca la intensidad en cada punto y se devuelve el color.
	Pero antes se debe multiplicar por el Kt del objeto que le sigue
*/
Color getTransparencia(Intersection *inter, int nivel, Point3D origin, Point3D direccion) {
	...
	if(tmp = inter) {
		if(nivel && (inter->obj->Kt > EPSILON)) {	// Si hay mas niveles y el objeto permite pasar la luz
			while(tmp && tmp->obj == inter->obj)
				tmp = tmp->next;
			agregar = getTransparencia(tmp, nivel - 1, origin, direccion);
		}
		point.x = origin.x + inter->t * direccion.x;
		point.y = origin.y + inter->t * direccion.y;
		point.z = origin.z + inter->t * direccion.z;
		normal = getNormal(point, inter->obj);
		dotProduct = normal.x * direccion.x + normal.z * direccion.z + normal.y * direccion.y;
		if(acos(dotProduct) * RADIAN < 90.0) {	// Hay que arreglar la normal
			normal.x *= -1;
			normal.y *= -1;
			normal.z *= -1;
		}
		intensity_and_phong = getIntensityAndPhong(point, direccion, normal, inter->obj);
		// Ya tenemos la intensidad en este objeto.
		//printf("Intensidad %.6f\n", intensity_and_phong.intensity);
		color.r = inter->obj->color.r * intensity_and_phong.intensity + intensity_and_phong.phong + agregar.r * inter->obj->Kt;
		color.g = inter->obj->color.g * intensity_and_phong.intensity + intensity_and_phong.phong + agregar.g * inter->obj->Kt;
		color.b = inter->obj->color.b * intensity_and_phong.intensity + intensity_and_phong.phong + agregar.b * inter->obj->Kt;
		//printf("Color del Objeto %.4f %.4f %.4f\n", inter->obj->color.r, inter->obj->color.r, inter->obj->color.r);
	} else color = background;
	//printf("Color Devuelto: %.4f %.4f %.4f\n", (double)color.r, (double)color.g, (double)color.b);
	return color;
}
...
int puntosEnPlano(Plane plane, PointList pl) {
	...
	pol_p1.x = max.x; pol_p1.y = max.y; pol_p1.z=0.0;
