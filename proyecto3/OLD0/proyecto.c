...
#define EPSILIN_ANTIALIASING  5.1961524
//#define EPSILIN_ANTIALIASING  2.5980762
//#define EPSILIN_ANTIALIASING  0.5980762
...
int H_SIZE, V_SIZE;
FILE *output;
int nivel_reflexion, nivel_transparencia, nivel_anti = 10;
...
Color getColor_AntiAliasign(Color a, Color b , Color c, Color d, double i, double j , int level, double factor) {
	...
	//Calcula la distancia entre los colores, para ello primero normaliza
	{
		Color a1=a,b1=b,c1=c,d1=d;
		L = (sqrt (a.r*a.r +  a.g*a.g + a.b*a.b) ); a.r = a.r / L; a.g = a.g / L; a.b = a.b / L;
		L = (sqrt (b.r*b.r +  b.g*b.g + b.b*b.b) ); b.r = b.r / L; b.g = b.g / L; b.b = b.b / L;
		L = (sqrt (c.r*c.r +  c.g*c.g + c.b*c.b) ); c.r = c.r / L; c.g = c.g / L; c.b = c.b / L;
		L = (sqrt (d.r*d.r +  d.g*d.g + d.b*a.b) ); d.r = d.r / L; d.g = d.g / L; d.b = d.b / L;
	...
	if ((L < EPSILIN_ANTIALIASING) && (level)) {
		//Promedio y es el nuevo color
		printf ("[%f][%f]Promedio en Nivel (%i) \t",i,j,level);
		return rate;
	} else {
		Point3D ptemp;
		...
		printf ("[%f][%f]Cuadrante en Nivel (%i) \t",i,j,level);
		...
		for (j=lastX;j<V_SIZE;j++){
			...
			for (i=0,k=0;i<H_SIZE;i++,k++){
				...
				*/
				...
				if ((L < EPSILIN_ANTIALIASING) && (nivel_anti)) {
				Color rate = {	(actfila[k].r+actfila[k+1].r+c.r+d.r) /4.0,
					...
					buffer[i][j] = getColor_AntiAliasign(actfila[k], actfila[k+1] , c, d, i, j , nivel_anti-1, 0.5);
