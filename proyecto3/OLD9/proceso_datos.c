...
void defaults() {
	title = "Computer Graphics: Ray Tracer";
	...
	peye.x = peye.y = 0.0;
	peye.z = -1000.0;
	nivel_reflexion = 0; nivel_transparencia = 0; nivel_anti = -1;
	...
	stretch_height_default = 100.0; stretch_width_default = 100.0;
}
...
void setWindow(double minx, double miny, double maxx, double maxy) {
	...
	//printf("Window [%.3f %.3f %.3f] [%.3f %.3f %.3f]\n", ventana.pmin.x, ventana.pmin.y, ventana.pmin.z,
	//										ventana.pmax.x, ventana.pmax.y, ventana.pmax.z);
}
...
void setTransparencia(int t) {
	...
	//printf("Nivel de Transparencia %i\n", nivel_transparencia);
}
...
void nuevoObjeto() {
	...
	tmp_obj->mapa_altura = tmp_obj->mapa_brillo = tmp_obj->mapa_transparencia = tmp_obj->mapa_calado = tmp_obj->mapa_especular = -1;
	//puntos_planos = NULL;
}
